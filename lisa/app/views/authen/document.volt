...
#search_button{
  ...
  {{ hidden_field('limit', 'value':1000, 'min':0) }}
  {{ hidden_field('offset',  'value': 0, 'min' : 0) }}
  <?php $this->flashSession->output() ?>
  <div style="font-size: 48px; font-family: RSU_BOLD;">รายการเอกสาร</div>
  {{ form ('lisa/authen/document', 'method' : 'post', "accept-charset":"utf-8") }}
  <div id="filterBar">
    ...
    <input id="endDate" name="endDate" placeholder="ถึงวันที่" style="width: 200px; font-size: 24px;" />
     ...
     {{ hidden_field('limit', 'value':1000, 'min':0) }}
     {{ hidden_field('offset',  'value': 0, 'min' : 0) }} <br>
     {{ submit_button('ค้นหา', 'class':'', 'id':'search_button') }}
   </div>
   {{ endform() }}
    <script>
      ...
      $('#endDate').datepicker({
        ...
        <table id="maintable" class="display" style="width:100%">
          ...
          <tbody style="text-align: start;">
            {% for val in viewDocuments.data %}
            <tr>
              <td>{{ val.documentId }}</td>
              <td>{{ val.title }}</td>
              <td>{{ val.sendAddress }}</td>
              <td>{{ val.receiver }}</td>
              <td>{{ substrDate(val.dateWrite) }}</td>
              <td class="text-center">
                <a id="viewDetail" href="{{url("lisa/authen/viewDetail")}}?doc_id={{ val.documentId }}" )}" class="btn"
