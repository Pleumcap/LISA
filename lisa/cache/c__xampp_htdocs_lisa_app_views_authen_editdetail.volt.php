    ...
    <link href="https://unpkg.com/gijgo@1.9.13/css/gijgo.min.css" rel="stylesheet" type="text/css" />
        ...
        <?= $this->tag->form(['lisa/authen/editDetail', 'method' => 'post']) ?>   
        <div style="margin-left: 120px;">
            <div style="display: flex;">
                <div style="color: #BCBCBC; font-size: 24px; font-family: RSU_BOLD;" >ชื่อเอกสาร</div>
                <div style="display: flex; text-align: center;">
                    <?= $this->tag->submitButton(['บันทึก', 'class' => 'menuOption', 'id' => 'saveButton']) ?>
                </div>
            </div>
        ...
        <div class="detailForm row detail">
            ...
            <div class="col-9">
                <?= $this->tag->textField(['title', 'class' => 'detailBorder', 'value' => $editDetail->data->title]) ?>
                ...
                <?= $this->tag->hiddenField(['documentId', 'class' => 'detailBorder', 'value' => $editDetail->data->documentId]) ?>
                <?= $this->tag->textField(['receiver', 'class' => 'detailBorder', 'value' => $editDetail->data->receiver]) ?>
                <?= $this->tag->textField(['sendAddress', 'class' => 'detailBorder', 'value' => $editDetail->data->sendAddress]) ?>
            </div>
        </div>
        ...
        <table style="width: 1500px; padding: 50px; text-align: center; border-radius: 46px;">
            ...
            <tr>
                <?= $this->tag->hiddenField(['personId[]', 'class' => '', 'value' => $i->personId]) ?>
                <?= $this->tag->hiddenField(['signatureImg[]', 'class' => '', 'value' => $i->signatureImg]) ?>
                <td><?= $this->tag->textField(['personName[]', 'class' => '', 'value' => $i->personName, 'style' => 'width:400px']) ?></td>
                <td><?= $this->tag->textField(['personRole[]', 'class' => '', 'value' => $i->personRole]) ?></td>
            </tr>
            <?php } ?>
            <?php } else { ?>
            <tr>
