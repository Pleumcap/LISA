...
<div style="font-size: 48px; font-family: RSU_BOLD;">รายการผู้ใช้งาน</div>
  <table id="maintable"  style="width:100%">
    ...
    <tbody>
      <?php foreach ($viewAllUser->data as $val) { ?>
      <tr>
          <td><?= $val->userId ?></td>
          ...
          <div id="dataModal<?= $val->userId ?>" class="modal" style="z-index: 1100;">
            ...
            <div class="modal-content">
              ...
              <div class="input-group">
                <?= $this->tag->hiddenField(['userId', 'value' => $val->userId]) ?>
              ...
              <?= $this->tag->submitButton(['บันทึก', 'class' => 'btn btn-success']) ?>
              <?= $this->tag->endform() ?>
            </div>
          </div>
          ...
          <input type="button" name="suspend" value="ระงับ" class="btn btn-danger suspend" data-id="<?= $val->userId ?>" style="margin: 10px;" ></input>
           ...
           <div id="suspendModal<?= $val->userId ?>" class="modal" style="z-index: 1100;">
            ...
            <div class="modal-content">
              ...
              <?= $this->tag->form(['lisa/authen/suspend', 'method' => 'post']) ?>
              <div class="input-group">
                <?= $this->tag->hiddenField(['userId', 'value' => $val->userId]) ?>
                <?= $this->tag->numericField(['days', 'size' => 32, 'value' => 0, 'min' => 0, 'id' => 'suspendTime']) ?>
                <label for="days"> : วัน</label>
                <?= $this->tag->numericField(['hours', 'size' => 32, 'value' => 0, 'min' => 0, 'id' => 'suspendTime']) ?>
                <label for="hours"> : ชั่วโมง</label>
                <?= $this->tag->numericField(['minutes', 'size' => 32, 'value' => 0, 'min' => 0, 'id' => 'suspendTime']) ?>
                <label for="minutes"> : นาที</label>
              </div>
              <div style="display: flex;">
                <input type="button" class="btn-danger suspendForever"value="ระงับบัญชีถาวร">
                <?= $this->tag->submitButton(['บันทึก', 'class' => 'btn btn-warning saveSuspend']) ?>
              </div>
              <?= $this->tag->endform() ?>
            </div>
          </div>
